 ...
 *  @version 0.1
 */
#ifndef __HAI_STRINGS_H__
#define __HAI_STRINGS_H__
#include "haibase.h"
...
public:
	 ...
	 *  @return 串行化后长度
	 */
	S_WORD Serialize(string &strData, const string &strdelimit)const;
	/** @brief 数据串行化(从后面开始)
	 *  @return 串行化后长度
	 */
	 ...
	 *  @return 串行化后长度
	 */
	S_WORD ReverseSerialize(string &strData, const string &strdelimit)const;
