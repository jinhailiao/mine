 ...
 *  @return 串行化后长度
 */
S_WORD strings::Serialize(string &strData, const string &strdelimit)const
{
	if (empty() == true)
		return 0x00;

	strData.append(*begin());
	for (strings::const_iterator iter = begin()+1; iter < end(); ++iter)
	{
		strData.append(strdelimit).append(*iter);
	}
	return (S_WORD)strData.size();
}
 ...
 *  @return 串行化后长度
 */
S_WORD strings::ReverseSerialize(string &strData, const string &strdelimit)const
{
	if (empty() == true)
		return 0x00;

	strData.append(*rbegin());
	for (strings::const_reverse_iterator iter = rbegin()+1; iter != rend(); ++iter)
	{
		strData.append(strdelimit).append(*iter);
	}
	return (S_WORD)strData.size();
